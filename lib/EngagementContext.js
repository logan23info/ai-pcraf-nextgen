...
export function EngagementProvider({ children, user, sb }) {
  ...
  // P14: Save active entity to localStorage for persistence across refreshes
  const saveToLocalStorage = useCallback(function(ent) {
    if (typeof window !== 'undefined' && ent?.id) {
      try { window.localStorage.setItem('pcraf_entity_id', ent.id) } catch(e) {}
    }
  }, [])
  ...
  const loadContext = useCallback(async () => {
    ...
    setLoading(true)
    try {
      // P14: Try to restore last active entity from localStorage
      let savedEntityId = null
      if (typeof window !== 'undefined') {
        try { savedEntityId = window.localStorage.getItem('pcraf_entity_id') } catch(e) {}
      }
      // Load entity — prefer localStorage-saved entity, fallback to latest
      let entQuery = sb.from('entities').select('*').eq('user_id', user.id)
      if (savedEntityId) entQuery = entQuery.eq('id', savedEntityId)
      else entQuery = entQuery.order('created_at', { ascending: false }).limit(1)
      const { data: entities } = await entQuery.limit(1)
      const ent = entities?.[0] || null
      setEntity(ent)
      if (ent) saveToLocalStorage(ent)
  ...
  // Wrap setEntity to also persist to localStorage
  const setEntityAndPersist = useCallback(function(ent) {
    setEntity(ent)
    saveToLocalStorage(ent)
  }, [saveToLocalStorage])

  const value = {
    entity, setEntity: setEntityAndPersist, mandateProfile, setMandateProfile,
