...
export function EngagementProvider({ children, user, sb }) {
  ...
  const loadContext = useCallback(async () => {
    ...
    try {
      ...
      // Load controls — filtered by active entity if one is loaded
      const ctrlQuery = sb.from('controls').select('*').eq('user_id', user.id)
      if (ent?.id) ctrlQuery.eq('entity_id', ent.id)
      const { data: ctrlData } = await ctrlQuery.order('created_at', { ascending: true })
