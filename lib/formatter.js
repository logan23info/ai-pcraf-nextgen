...
export function formatAIOutput(text) {
  ...
  lines.forEach(line => {
    ...
    // Match CoT step patterns: "Step 1 —", "STEP 1:", "CoT Step 1"
    const step = t.match(/^(?:CoT\s+)?Step\s+(\d+)\s*[\u2014:\-]+\s*(.+)$/i) ||
                 t.match(/^(Step\s+\d+[^a-z]*)\s+(.{10,})$/i)
    if (step) {
      closeList()
      const stepNum = step[1].toString().trim()
      const stepBody = step[2].toString().trim()
      html += '<div class="step-block"><div class="step-title">Step ' + stepNum + '</div><div class="step-body">' + proc(stepBody) + '</div></div>'
      return
    }
